...
func TestOptionalUUIDAnyValidation(t *testing.T) {
	testcases := []struct {
		uuid string
		fail bool
	}{
	...
	for _, tc := range testcases {
		t.Run(fmt.Sprintf("proto2 uuid '%s'", tc.uuid), func(t *testing.T) {
			msg := buildProto2("-%ab", 11, "abba", 99, 0.5, 0.5, 0.5, 0.5, "x", 4, "1234567890", stableBytes, tc.uuid, uuid4)
		...
		t.Run(fmt.Sprintf("proto3 uuid '%s'", tc.uuid), func(t *testing.T) {
			msg := buildProto3("-%ab", 11, "abba", 99, 0.5, 0.5, 0.5, 0.5, "x", 4, "1234567890", stableBytes, tc.uuid, uuid4)
